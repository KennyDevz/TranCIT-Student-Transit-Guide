document.addEventListener('DOMContentLoaded', () => {
    ...
    // === Auto-fill Hidden Inputs from URL ===
    (function syncInputsFromURL() {
        const params = new URLSearchParams(window.location.search);

        if (params.get('origin_latitude')) originLat.value = params.get('origin_latitude');
        if (params.get('origin_longitude')) originLon.value = params.get('origin_longitude');
        if (params.get('destination_latitude')) destLat.value = params.get('destination_latitude');
        if (params.get('destination_longitude')) destLon.value = params.get('destination_longitude');

        if (params.get('origin_text') && !originInput.value)
        originInput.value = decodeURIComponent(params.get('origin_text'));
        if (params.get('destination_text') && !destinationInput.value)
        destinationInput.value = decodeURIComponent(params.get('destination_text'));
    })();
    ...
    [originLat, originLon, destLat, destLon].forEach(input =>
        input?.addEventListener('change', toggleNavigateButton)
    );

    window.addEventListener('pageshow', toggleNavigateButton);
    window.addEventListener('load', toggleNavigateButton);
    ...
    saveBtn?.addEventListener('click', async () => {
        ...
        fd.append('origin', originInput.value || '');
        fd.append('destination', destinationInput.value || '');
        fd.append('transport_type', transportSelect?.value || '');
        fd.append('fare', fareInput?.value || '0');
        fd.append('distance_km', distInput?.value || '');
        fd.append('travel_time_minutes', timeInput?.value || '');
        fd.append('origin_latitude', originLat?.value || '');
        fd.append('origin_longitude', originLon?.value || '');
        fd.append('destination_latitude', destLat?.value || '');
        fd.append('destination_longitude', destLon?.value || '');
        fd.append('notes', notesInput?.value || '');
        fd.append('code', codeInput?.value || '');

        try {
        const url = `${window.location.pathname.replace(/\/$/, '')}/save_current_route/`;
        const res = await postJSON(url, fd);
        res?.id ? (alertMsg('Route saved!'), window.location.reload()) : alertMsg('Could not save route.');
        } catch {
        alertMsg('Error saving route.');
        }
    });
    ...
    routeForm?.addEventListener('submit', (e) => {
        ...
        $('#error-origin').textContent = 'Origin required';
        valid = false;
        }
        if (!destinationInput.value.trim()) {
        $('#error-destination').textContent = 'Destination required';
        valid = false;
        }
        if (!type) {
        $('#error-transport-type').textContent = 'Select transport';
        valid = false;
        }
        if (['Taxi', 'Motorcycle'].includes(type) && (!fareInput.value || +fareInput.value <= 0)) {
        fareDisplay.style.color = 'red';
        fareDisplay.textContent = 'Calculate fare!';
        valid = false;
        }
